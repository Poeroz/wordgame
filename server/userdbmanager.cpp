...
void userdbManager::writeJson(QJsonArray &json) const {
    QSqlQuery query(userdb);
    if (!query.exec("select * from user")) {
        qDebug() << "print error";
    }
        ...
        while (query.next()) {
            ...
            userObject["usr"] = query.value(USERNAME).toString();
            userObject["name"] = query.value(NICKNAME).toString();
            userObject["pwd"] = query.value(PASSWORD).toString();
            userObject["role"] = query.value(ROLE).toBool();
            userObject["grade"] = query.value(GRADE).toInt();
            userObject["level"] = query.value(LEVELCNT).toInt();
            userObject["exp"] = query.value(EXPERIENCE).toInt();
            userObject["qcnt"] = query.value(QUESTIONCNT).toInt();
            json.append(userObject);
        }
    }
}
